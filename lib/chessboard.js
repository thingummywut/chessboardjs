...
;(function () {
  ...
  CSS['overlay'] = 'overlay-4fc5e'
  CSS['overlayArrow'] = 'overlay-arrow-9d6ed'
  CSS['overlayGroup'] = 'overlay-group-672a1'
  ...
  function buildContainerHTML (hasSparePieces, hasOverlay) {
 ...
 */
  function computePath(isWhite, squareSize, s1, s2) {
	...
	var epsilon;  // To adjust the target coords to take account of the arrowhead.
      ...
      var xDist = Math.abs(dist.x);
      var yDist = Math.abs(dist.y);
      
      if((xDist >= 1 && xDist <= 2) && (yDist >=1 && yDist <=2) && xDist != yDist) {
	...
	var path = ["M", squareSize * (start.x + 0.5), squareSize * (start.y + 0.5)];
	if(corner !== undefined) {
		path.push("L", squareSize * (corner.x + 0.5) , squareSize * (corner.y + 0.5));
	}	
	path.push("L", squareSize * (end.x + epsilon.x + 0.5), squareSize * (end.y + epsilon.y + 0.5));
  ...
  function constructor (containerElOrString, config) {
    ...
    widget.resize = function () {
      ...
      $board.css('width', squareSize * 8 + 'px')
        ...
        $overlay.css('width', (squareSize * 8) + 'px');
        $overlay.css('height', (squareSize * 8) + 'px');
        $overlay.css('padding', (boardBorderSize) + 'px');
      // set drag piece size
      $draggedPiece.css({
        height: squareSize,
        width: squareSize
    ...
    widget.start = function (useAnimation) {
	  ...
	  if(config.overlay === true) {
	      var groupEl = $overlay.find('> .square-' + source);
	      if(!groupEl.length) {
		  groupEl = createSvgEl("g", {
		      'class': CSS['overlayGroup'] + " square-" + source
		  });
		  $overlay.append(groupEl);
	      }
	      
	      var pathEl = $overlay.find("> g.square-" + source + " > path.square-" + target);
              debugger;
	      if(!pathEl.length) {
		  var pathEl = createSvgEl("path", {
		      'class' :       CSS['overlayArrow'] + " square-" + target,
		      'd':            computePath((config.orientation === "white"), squareSize, source, target),
		      'stroke-width': (squareSize / 3)
		  });
      ...
      widget.clearAnnotation = function() {
          if (config.hasOwnProperty("overlay") === true) {
    ...
    function initDOM () {
      ...
      // build board and save it in memory
        $container.html(buildContainerHTML(config.sparePieces, config.overlay))
      $board = $container.find('.' + CSS.board)
      $overlay = $container.find('.' + CSS.overlay)
        if (config.hasOwnProperty("overlay") === true) {
            buildOverlay($overlay);  
        }
